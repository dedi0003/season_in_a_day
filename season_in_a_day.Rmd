...
* Wear our jacket during night and early morning, and pull it off during afternoon
--

* People are true when they say that there are 4 seasons in a day in Melbourne!

.pull-left[
```{r season, eval=FALSE, message=FALSE, warning = FALSE}
humi <- melb_temp %>% 
  filter(type == "TPH.RH-EPA-1h") %>% 
  group_by(hour) %>% 
  summarise(ave_humid = mean(value, na.rm = TRUE)) 
...
temp <- melb_temp %>% 
 ...
 ggplot() + geom_col(data = humi, aes(x = hour, y = ave_humid, fill = "humid")) +  
...
.pull-right[
```{r plot-out3, ref.label= "season", echo = FALSE, warning = FALSE, message=FALSE, out.width= "90%"}
